...
async function cli(args) {
  ...
  }).command("env", "set environment variables", {
    builder: (yargs2) => yargs2.option("app", {
      alias: ["a"],
      default: process.env.DEPLOYER_APP,
      demandOption: true,
      description: "App name"
    }),
    handler: async ({_: args2, app}) => {
      const manager = await deployer.connect(app);
      if (args2[0] === "env")
        args2.shift();
      if (args2.length === 0) {
 ...
 - ${manager.app.env.join("\n - ")}`);
        ...
        const envs = Object.fromEntries(args2.filter((arg) => typeof arg === "string").map((arg) => {
          const index = arg.indexOf("=");
          if (index >= 0) {
            return [arg.substr(0, index), arg.substr(index + 1)];
          } else {
            return [arg, "true"];
          }
        }));
