    ...
    var AppManager = class {
      ...
      async createRelease(options, asset) {
        ...
        if (stat.isFile()) {
          ...
          assetFilename = Path__namespace2.join(await FS__namespace2.promises.mkdtemp(Path__namespace2.join(OS__namespace.tmpdir(), "deployer-")), "archive.tar.gz");
          shouldUnlinkAssetFile = true;
          const ignoreFiles = [
            Path__namespace2.resolve(asset, ".deployignore"),
            Path__namespace2.resolve(process.cwd(), ".deployignore")
          ].filter((file) => FS__namespace2.existsSync(file));
        ...
        return this.withManifestSync(async () => {
          ...
          if (shouldUnlinkAssetFile)
            await FS__namespace2.promises.unlink(assetFilename);
