...
// lib/DeploymentManager-47dc9463.js
var require_DeploymentManager_47dc9463 = __commonJS({
  "lib/DeploymentManager-47dc9463.js"(exports2) {
    "use strict";
    ...
    var AppManager = class {
      ...
      async createRelease(options, asset) {
        ...
        if (stat.isFile()) {
          ...
          assetFilename = Path__namespace.join(await FS__namespace.promises.mkdtemp(Path__namespace.join(OS__namespace.tmpdir(), "deployer-")), "archive.tar.gz");
          shouldUnlinkAssetFile = true;
          const ignoreFiles = [
            Path__namespace.resolve(asset, ".deployignore"),
            Path__namespace.resolve(process.cwd(), ".deployignore")
          ].filter((file) => FS__namespace.existsSync(file));
        ...
        return this.withManifestSync(async () => {
          ...
          if (shouldUnlinkAssetFile)
            await FS__namespace.promises.unlink(assetFilename);
...
`);
      ...
      async upload(localFile, remoteFile) {
        try {
          const {stdout, stderr} = await Util__namespace.promisify(CP__namespace.exec)(`scp -q -i ${quote(this.server.privateKey)} -P ${this.server.port} ${quote(localFile)} ${this.server.user}@${this.server.host}:${quote(remoteFile)}`);
          if (stderr !== "")
      ...
      async exec(script) {
        try {
          ...
          const {stdout, stderr} = await Util__namespace.promisify(CP__namespace.exec)(`ssh -q -i ${quote(this.server.privateKey)} -p ${this.server.port} ${this.server.user}@${this.server.host} <<'${boundary}'
...
var DeploymentManager = require_DeploymentManager_47dc9463();
...
require("path");
require("util");
exports.AppManager = DeploymentManager.AppManager;
exports.DeploymentManager = DeploymentManager.DeploymentManager;
exports.SSHClient = DeploymentManager.SSHClient;
exports.parseAppInstanceId = Id.parseAppInstanceId;
exports.parseReleaseId = Id.parseReleaseId;
