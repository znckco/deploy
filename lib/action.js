...
// lib/DeploymentManager-7aec29f9.js
var require_DeploymentManager_7aec29f9 = __commonJS({
  "lib/DeploymentManager-7aec29f9.js"(exports2) {
    "use strict";
    ...
    var AppManager = class {
      ...
      async createRelease(options, asset) {
        ...
        return this.withManifestSync(async () => {
          ...
          const deleteReleases = new Set();
          keepReleases.forEach((release2) => activeReleaseIds.delete(release2.id));
          this.app.releases.forEach((release2) => {
            if (activeReleaseIds.has(release2.id)) {
              keepReleases.push(release2);
            } else {
              deleteReleases.add(release2.id);
            }
          });
          await this.sh.exec(`rm -f ${Array.from(deleteReleases).map((id) => quote(`${this.appDir}/releases/v${id}.tar.gz`))}`);
    ...
    var SSHClient = class {
      constructor(server) {
        ...
        if (process.env.DEPLOYER_SSH_KEY != null) {
          ...
          FS__namespace.writeFileSync(this.server.privateKey, process.env.DEPLOYER_SSH_KEY);
        }
      }
      async ls(expr) {
...
`);
      ...
      async upload(localFile, remoteFile) {
        try {
          const {stdout, stderr} = await Util__namespace.promisify(CP__namespace.exec)(`scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q -i ${quote(this.server.privateKey)} -P ${this.server.port} ${quote(localFile)} ${this.server.user}@${this.server.host}:${quote(remoteFile)}`);
          if (stderr !== "")
      ...
      async exec(script) {
        try {
          ...
          const {stdout, stderr} = await Util__namespace.promisify(CP__namespace.exec)(`ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q -i ${quote(this.server.privateKey)} -p ${this.server.port} ${this.server.user}@${this.server.host} <<'${boundary}'
...
require("os");
require("util");
var DeploymentManager = require_DeploymentManager_7aec29f9();
  ...
  switch (action) {
    case "create-release":
      return {
        ...
        commit: (_a = Action__default["default"].getInput("commit")) !== null && _a !== void 0 ? _a : GitHub__default["default"].context.sha,
        tag: (_b = Action__default["default"].getInput("tag")) !== null && _b !== void 0 ? _b : "latest",
        artefact: (_c = Action__default["default"].getInput("artefact")) !== null && _c !== void 0 ? _c : process.cwd()
      };
...
async function main() {
  ...
  const configFile = Path__default["default"].resolve(workingDir, (_a = Action__default["default"].getInput("deploy_config")) !== null && _a !== void 0 ? _a : "deploy.json");
