...
type ReleaseId = OpaqueId<"Release", number>;
type AppInstanceId = OpaqueId<"AppInstance", string>;
...
function areObjectsEqual(a: any, b: any, depth: number = 0): boolean {
...
class AppManager {
  ...
  // -- Release Management
  async createRelease(options: Pick<Release, "commit" | "version">, asset: string): Promise<Release> {
    ...
    if (stat.isFile()) {
      ...
      const { stderr } = await Util.promisify(CP.exec)(
        `tar -c ${extraArgs} -zf ${quote(assetFilename)} ${quote(asset)}`,
        {
    ...
    return this.withManifestSync(async () => {
      ...
      this.log(
        `Create release v${release.id} from ${this.app.repository}/commit/${release.commit} (version: ${release.version})`,
      );
  ...
  async createInstance(releaseId: ReleaseId): Promise<AppInstance> {
    ...
    return this.withManifestSync(async () => {
      const id = uuid() as AppInstanceId;
  ...
  async status(instanceId: AppInstanceId): Promise<string> {
  ...
  async destroy(instanceId: AppInstanceId): Promise<void> {
...
function getCreatedAt(): DateString {
...
export async function cli(args: string[]): Promise<void> {
  ...
  yargs
    ...
    .command<{ app: string; commit: string; version: string; artefact: string }>(
      "releases create",
...
async function importJSON<T>(configFile: string): Promise<T> {
